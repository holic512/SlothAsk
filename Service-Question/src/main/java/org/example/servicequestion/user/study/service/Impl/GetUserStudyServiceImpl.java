    ...
    public Page<Question> getQuestionList(Long projectId, GetQuestionListRequest request) {
        ...
        for (int i = 0; i < questionList.size(); i++) {
            ...
            if (virtualId == null) {
                ...
                virtualIdMap.put(VidKey + virtualId, String.valueOf(originalId));
            }
            
            // 设置虚拟ID和序号
            question.setVirtualId(virtualId);
            question.setId((long) startIndex++);
        }
        
        // 4. 批量存储新生成的虚拟ID到Redis
