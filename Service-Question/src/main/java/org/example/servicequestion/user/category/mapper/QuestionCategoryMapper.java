...
import org.example.servicequestion.user.category.dto.QuestionListDto;
...
public interface QuestionCategoryMapper extends BaseMapper<QuestionCategory> {
    ...
    @Select("SELECT id, name, description, creator_id, avatar_url, sort_order, view_count, status, create_time, update_time, project_id " +
            ...
            "WHERE status = 1 and project_id = 1 " +
    ...
    @Select("select id, name from question_category where status = 1 and project_id = #{projectId}")
    List<CategoryIdAndNameDto> getCategoryIdAndNameByProjectId(@Param("projectId") Long projectId);

    @Select("select id, name from question_tag_category where status = 1 and project_id = #{projectId}")
    List<TagIdAndNameDto> getTagIdAndName(@Param("projectId") Long projectId);
