...
package org.example.serviceai.commonService;

import org.example.servicecommon.util.IdEncryptor;
...
import static org.example.servicecommon.redisKey.QuestionKey.QUESTION_VID_KEY;
...
public class IdConversionService {
    ...
    public Long getOriginalIdFromVirtualId(String virtualId) {
        String originalIdStr = (String) redisTemplate.opsForValue().get(QUESTION_VID_KEY + virtualId);
        ...
        if (originalId == null) {
            ...
            if (originalId != null) {
                redisTemplate.opsForValue().set(QUESTION_VID_KEY + virtualId, String.valueOf(originalId), 1, TimeUnit.DAYS);
                redisTemplate.opsForValue().set(QUESTION_VID_KEY + originalId, virtualId, 1, TimeUnit.DAYS);
            }
        }
    ...
    public String getVirtualIdFromOriginalId(Long originalId) {
        String virtualId = (String) redisTemplate.opsForValue().get(QUESTION_VID_KEY + originalId);
